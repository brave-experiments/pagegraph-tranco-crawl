...
from pgcrawl.dispatch.actions import setup_client_code, domains_to_crawl, crawl
...
def client_crawl(ips: list[IPAddress], user: UserName, summarize: bool,
    ...
    manager = ThreadIPManager(ips, user, client_crawl_args.timeout, quiet)
    with ThreadPoolExecutor(max_workers=manager.num_workers(),
                            initializer=manager.init_thread,) as executor:
        message = f"Crawling {len(domains)} domains"
        log(message, quiet)

        for domain_request in domains:
            work_item = (crawl, message, [domain_request,
